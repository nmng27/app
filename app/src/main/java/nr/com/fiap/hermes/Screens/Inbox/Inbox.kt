...
@Composable
fun Inbox(navController: NavController,usuarioLogado:String,themeManager: ThemeManager) {
    var listaEmails:List<Email> by remember {
        mutableStateOf(listOf<Email>())
    }
    val isDarkTheme = themeManager.isDarkTheme

    val call = RetrofitFactory().getEmailService().listarPorCategoria("Inbox",usuarioLogado)
    call.enqueue(
